...
def load_data(filename):
    """读取全局棋谱
    返回: [(棋谱, 结果)], 
    其中结果等于2为红方赢棋, 1为和棋, 0为黑方赢棋, -1则为无明确标注胜负.
    """
    D = []
...
bert_token_dict = load_vocab(dict_path_wwm_zh)
...
class data_generator(DataGenerator):
    ...
    def __iter__(self, random=False):
        ...
        for is_end, (text, label) in self.sample(random):
            token_ids, segment_ids = tokenizer.encode(' '.join(text), maxlen=maxlen // self.n + 1)
