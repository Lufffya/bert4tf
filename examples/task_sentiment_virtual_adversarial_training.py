...
def virtual_adversarial_training(model, embedding_name, epsilon=1, xi=10, iters=1):
    ...
    model_outputs = K.function(inputs=inputs, outputs=model.outputs, name='model_outputs') # 模型输出函数
    embedding_gradients = K.function(inputs=inputs, outputs=[gradients], name='embedding_gradients') # 模型梯度函数
